...
const COPIES = 5;
const MID_COPY = Math.floor(COPIES / 2); // 2
const TOTAL = LEN * COPIES;

const DEFAULT_SECTOR_INDEX = 0; // Agriculture & Trade
...
function SectorCard({ sector, isActive, onActivate, onOpen }) {
    const { title, description, icon: Icon } = sector;
    ...
    return (
        <motion.div
            ...
            onClick={onActivate}
            className={`
        flex flex-col flex-shrink-0 cursor-pointer overflow-hidden
        rounded-[24px] bg-[#0a3448]

        h-[420px] w-[calc(100vw-48px)] max-w-[390px]

        sm:h-[480px] sm:w-[420px]

        lg:h-[520px] lg:w-[400px] lg:flex-row

        ${isActive ? "max-[767px]:h-[560px] lg:w-[1180px] lg:max-w-[85vw]" : ""}
      `}
        >
            {isActive && (
                <motion.div
                    ...
                    className="h-[48%] w-full flex-shrink-0 lg:h-full lg:w-[62%]"
                >
            ...
            <div
                className={`
          flex min-w-0 flex-1 flex-col justify-between p-7
          sm:p-8 lg:p-10
          ${isActive ? "h-[52%] lg:h-full" : "h-full"}
        `}
            >
                <div>
                    <motion.h3
                    ...
                    <motion.p
                        layout="position"
                        className="mt-3 max-w-[420px] text-sm leading-6 text-white/70 sm:mt-4 sm:text-[15px]"
                    >
                        {description}
                    </motion.p>
                </div>
                ...
                <button
                    ...
                    className="
            w-fit rounded-full bg-white px-5 py-3 text-sm font-medium
            text-[#0a3448] transition-all duration-300

            hover:scale-[1.02] hover:opacity-90 active:scale-95
            sm:px-6
          "
                >
                    Read More
...
function SectorModal({ sector, onClose }) {
    ...
    return (
        ...
        bg-[#0a1a24]/70 p-3 backdrop-blur-md sm:p-6
      "
        >
            <motion.div
                ...
                className="
          relative flex h-full max-h-[92vh] w-full max-w-5xl flex-col
          overflow-hidden rounded-[24px] bg-white
          shadow-[0_30px_100px_rgba(0,0,0,0.35)]

          md:h-auto md:max-h-[88vh] md:flex-row
        "
            >
                <button
                    ref={closeButtonRef}
            ...
            justify-center rounded-full bg-white/85 text-[#0a3448]
          ...
          "
                ...
                <div className="relative h-48 w-full flex-shrink-0 overflow-hidden sm:h-64 md:h-auto md:w-[42%]">
                    <img src={image} alt={title} className="h-full w-full object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#04202e]/70 via-transparent to-transparent md:bg-gradient-to-r md:from-transparent md:via-transparent md:to-[#04202e]/20" />

                    <div
                        className="
              absolute bottom-4 left-4 flex h-12 w-12 items-center
              justify-center rounded-2xl border border-white/30 bg-white/15
            ...
            "
                ...
                <div
                    className="
            flex min-w-0 flex-1 flex-col overflow-y-auto px-6 py-7
            sm:px-8 sm:py-9 md:px-10 md:py-12
          "
                >
                    <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[#7C9473]">
                        Verdant Vale Group
                    </p>
                    ...
                    <h2
                        ...
                        className="
              mt-2 text-2xl font-light leading-tight tracking-tight text-[#0B1F33]
            ...
            "
                    ...
                    <p className="mt-3 text-sm leading-6 text-neutral-500 sm:text-[15px] sm:leading-7">
                        {description}
                    </p>

                    <div className="mt-6 space-y-5 text-[15px] leading-7 text-neutral-700 sm:mt-8">
                    ...
                    <div className="mt-8 border-t border-neutral-200 pt-6">
                        <button
                            ...
                            onClick={onClose}
                            className="
                w-fit rounded-full bg-[#0a3448] px-6 py-3 text-sm font-medium
...
export default function Sectors() {
    ...
    // Timer used to snap the track after the user stops scrolling on touch.
    ...
    const jumpToCard = useCallback((trackIdx) => {
        const track = trackRef.current;
        const centers = centersRef.current;
        if (!track || centers[trackIdx] == null) return;
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            if (isJumpingRef.current || isSmoothRef.current) return;
            ...
            /*
             * Schedule a snap AFTER the user stops scrolling.
             * On touch devices, momentum scrolling ends without any event
             * telling us "scrolling is done" — so we wait for a short pause
             * in scroll events and then smooth-scroll the nearest card to
             * the exact center.
             */
            ...
            snapTimerRef.current = window.setTimeout(() => {
                ...
                // Re-measure in case a card just expanded/shrank.
                measureCenters();

                const idx = nearestCardIndex();
                const centers = centersRef.current;
                if (centers[idx] == null) return;

                const target = Math.max(0, centers[idx] - track.clientWidth / 2);

                // Only snap if we're meaningfully off-center (>4px).
    ...
    /* ------------------------------------------------------------------------ */
    /* Activate a sector — clicking the card body (no modal)                    */
    /* ------------------------------------------------------------------------ */
    ...
    /* ------------------------------------------------------------------------ */
    /* Open a sector — clicking "Read More" (activates + opens modal)           */
    /* ------------------------------------------------------------------------ */
    ...
    return (
        ...
        scroll-mt-16 px-6 pt-12 pb-20
        sm:scroll-mt-[72px] sm:px-8 sm:pt-14 sm:pb-24
        lg:scroll-mt-20
      "
        >
            {/* Heading */}
            ...
            >
                <h1
                    className="
            text-3xl font-light leading-[1.15] tracking-tight text-neutral-900
          ...
          "
                ...
                <p
                    className="
            mx-auto mt-5 max-w-[620px] text-sm leading-6 text-neutral-600
            sm:mt-6 sm:text-[16px] sm:leading-7
          "
                >
            ...
            <div className="relative mt-12 w-full sm:mt-16">
                <div
                    ref={trackRef}
                    className="
            flex w-full gap-4 overflow-x-auto px-6 pb-4
            [scrollbar-width:none] [&::-webkit-scrollbar]:hidden
            overscroll-x-contain
            sm:gap-6 sm:px-8
          "
                >
                    {renderedCards.map(({ key, sector, isActive, trackIdx, realIndex }) => (
              ...
              rounded-full border border-neutral-300 text-neutral-700
